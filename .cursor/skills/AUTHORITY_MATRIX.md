...
| Day-to-day ops, roadmap priorities, lifecycle tracking, blockers, AI spend watch, ops/milestone reports | **Operations Director** subagent (`.cursor/agents/operations-director.md`) | `/ceo` for go/no-go; Engineering Director for impl coordination; `/release-manager` + `/qa-breaker` + `/security-auditor` before release; Hub Architect for Hub design | Implement production code; approve releases without RM+QA+Security; claim complete without evidence |
...
| Operations Director | `.cursor/agents/operations-director.md` | Day-to-day ops, lifecycle, blockers, spend/Review Gate watch, daily/milestone ops reports |
