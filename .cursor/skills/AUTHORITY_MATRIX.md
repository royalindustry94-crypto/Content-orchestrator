...
|---|---|---|---|
| Product scope, quality bar, go/no-go, VERIFIED label | `/ceo` | Specialists for evidence | Implement specialist deep work as substitute for delegation |
...
| FastAPI/worker code, DI, auth wiring, tests for app behavior | `/backend-engineer` | `/chief-architect` before arch change; `/postgresql-expert` before schema/RLS/migration design | Invent architecture; declare migration-safe without PG review |
...
2. Mark work **VERIFIED** or **COMPLETE** without **factual evidence** (CI URL + SHA, migration head, test counts, adversarial RLS results, audit defects list).
...
4. Skip **QA** (full `pytest -W error`, ruff, migration replay) or **security** review (authz/RLS/secrets/audit) before release claims.
