...
`#/template` (also “View neon dashboard template” on sign-in) opens an original five-phone kit on a dark grid — Dashboard, Stats, Transfer, Planning, History — in charcoal `#121212` / panel `#1C1C21` / magenta `#FF007A` / orange `#FF9A00` / cyan `#00D9FF` / lime `#B8F54A`. Sample figures such as `$1,490.00` are decorative template chrome and never appear on Home Bankroll.
