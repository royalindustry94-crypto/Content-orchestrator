...
| Effort | S |
| Status | **CLOSED.** Self-measured and self-closed in this pass; this is a bounded CI-policy tuning task (no application code, tests, or migrations touched), not a milestone, so it doesn't require a separate independent audit before merge, but an independent re-run of `pytest --cov=app --cov-fail-under=79` is still welcome. |
