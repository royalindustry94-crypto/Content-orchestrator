...
| Test baseline (before formatting) | `apps/api`: **359 passed**, 81.06% coverage (`pytest --cov=app --cov-fail-under=75`, floor 75%, env matching CI's `api` job exactly: local Postgres 16, `content_orchestrator_test` DB, `app_runtime` role, `AUTH_MODE=local`). `apps/worker`: **7 passed** (`pytest`). |
| Test baseline (after formatting, final) | `apps/api`: **359 passed** — identical pass count to baseline (coverage 81.04% vs. baseline 81.06%; the 2-line shift in statements-missed is `coverage.py` re-attributing lines after `ruff format` collapsed/split some multi-line statements onto different source lines — a measurement artifact, not a behavior change, and still well clear of the 75% floor). `apps/worker`: **7 passed** — identical to baseline. `ruff format --check .` reports **0 files would be reformatted** (fully clean) in both directories. `ruff check .` clean in both. |
