...
| Test baseline (after formatting, final) | `apps/api`: **359 passed**, 81.06% coverage — identical to baseline. `apps/worker`: **7 passed** — identical to baseline. `ruff format --check .` reports **0 files would be reformatted** (fully clean) in both directories. `ruff check .` clean in both. |
