...
async def mark_stale_workers_offline(
    ...
    *,
    ...
    is older than the threshold to OFFLINE and zero their load. Single
    idempotent UPDATE — safe to run concurrently with heartbeats (row
    locks serialize; a live worker's next heartbeat flips it back).
    Deregistered rows are already offline (DB check constraint) and are
    excluded to keep the sweep on the partial live index.
