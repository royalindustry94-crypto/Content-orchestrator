...
class ResearchSource(Base, WorkspaceScopedMixin, CreatedAtMixin):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "research_run_id", "canonical_url", name="uq_research_source_run_url"),
...
class Opportunity(Base, WorkspaceScopedMixin, TimestampMixin, VersionMixin, ActorMixin, SoftDeleteMixin):
    ...
    performance_data_state: Mapped[str] = mapped_column(Text, nullable=False, default="no_performance_data")
...
class OpportunityEvidence(Base, WorkspaceScopedMixin, CreatedAtMixin):
    ...
    __table_args__ = (
        UniqueConstraint("workspace_id", "opportunity_id", "source_id", name="uq_opportunity_evidence_link"),
        Index("ix_opportunity_evidence_workspace_opportunity", "workspace_id", "opportunity_id"),
...
class ResearchAudit(Base, WorkspaceScopedMixin, CreatedAtMixin):
    ...
    __table_args__ = (
        Index("ix_research_audits_workspace_opportunity_checked", "workspace_id", "opportunity_id", "checked_at"),
    ...
    evaluator_context_version: Mapped[str] = mapped_column(Text, nullable=False, default="research-auditor-v1")
...
class ResearchSchedule(Base, WorkspaceScopedMixin, TimestampMixin, VersionMixin, ActorMixin):
    ...
    enabled_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("profiles.id"), nullable=True)
