...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint(
            "max_searches > 0 AND max_provider_calls >= 0 AND max_tokens >= 0 AND max_cost_usd >= 0 AND max_attempts > 0",
            name="ck_research_runs_bounds",
        ),
    )
    op.create_index(
        "ix_research_runs_workspace_created", "research_runs", ["workspace_id", "created_at"]
    )
    op.create_index(
        "ix_research_runs_workspace_status", "research_runs", ["workspace_id", "status"]
    )
    op.create_index(
        "ix_research_runs_workspace_correlation",
        "research_runs",
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "workspace_id", "research_run_id", "canonical_url", name="uq_research_source_run_url"
        ),
    )
    op.create_index(
        "ix_research_sources_workspace_run", "research_sources", ["workspace_id", "research_run_id"]
    )
    op.create_index(
        "ix_research_sources_workspace_digest",
        "research_sources",
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text(),
            nullable=False,
            server_default="no_performance_data",
        ),
    ...
    op.create_index(
        "ix_opportunities_workspace_status", "opportunities", ["workspace_id", "status"]
    )
    op.create_index(
        "ix_opportunities_workspace_run", "opportunities", ["workspace_id", "research_run_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "contradiction_flag", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
        ...
        sa.UniqueConstraint(
            "workspace_id", "opportunity_id", "source_id", name="uq_opportunity_evidence_link"
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text(),
            nullable=False,
            server_default="research-auditor-v1",
        ),
        sa.Column(
            "scout_snapshot",
            postgresql.JSONB(astext_type=sa.Text()),
    ...
    op.create_index(
        ...
        "research_audits",
        ["workspace_id", "opportunity_id", "checked_at"],
    )
    op.create_index(
        "ix_research_audits_workspace_run", "research_audits", ["workspace_id", "research_run_id"]
    )
