...
async def test_pipeline_run_idempotency_key_unique_per_workspace():
    async with AsyncSessionLocal() as s:
        ...
        await s.execute(
            text(
                "INSERT INTO pipeline_runs (workspace_id, content_item_id, idempotency_key) "
...
async def test_content_lineage_is_immutable():
    async with AsyncSessionLocal() as s:
        ...
        await s.execute(
            text(
                "INSERT INTO content_lineage (id, workspace_id, parent_content_item_id, "
...
async def test_asset_storage_and_provider_metadata_columns_exist():
    async with AsyncSessionLocal() as s:
        rows = await s.execute(
            text("SELECT column_name FROM information_schema.columns WHERE table_name = 'assets'")
        )
